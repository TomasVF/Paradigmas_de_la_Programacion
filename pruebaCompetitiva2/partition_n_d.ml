...
let add_smaller n (h::l) i a = match h with
    [] -> ((n::[])::l, i, true)
    |h1::t -> let rec aux n l l2 l3 w i a = match l with
                [] -> ((n::l3)::l2, 1, valid (n::l3) a)
                | h2::t2 -> let x = weight h2 in if x < w then aux n t2 (l3::l2) h2 x (i+1) a else aux n t2 (h2::l2) l3 w (i+1) a
              in aux n l [] h (weight h) i a
;;
(*lo mismo que valid pero aplicado a cada lista de una lista de listas*)
let rec valid_list l n = match l with
    []-> true
    |h::t -> weight h <= n && valid_list t n 
;;

(*añade a una lista de listas l el elemento n en la posición b, si b es cero se añade en la lista con menor peso de l, devuelve la lista con el elemento añadido, si se añadió en la lista más pequeña la posición de esta lista en la lista de listas y si el añadir este elemento en esa posición es válido*)
let add2 n l b i num = match l with
    [] -> ([], num, false)
    | h::t -> if b = 0 then add_smaller n l 1 i
                       else let rec aux n l l2 i a = match l with
                                [] -> (l2, num, a)
                                | h::t -> if i = b then  aux n [] (List.rev_append ((n::h)::l2) t) i (valid (n::h) i) else aux n t (h::l2) (i+1) false
                            in aux n l [] 1 false
;;

(*prueva si hay alguna forma de añadir a la lista de listas el elemento n llegando así una de sus listas al peso a*)
let try_with n l a = 
...
let rec aux2 i l n l2 b a num =
    ...
    else let (li, nu, vali) = add2 (List.hd l) l2 b n num in if not(vali) then aux2 i l n l2 (b+1) a nu else
    match aux2 i (List.tl l) n li 0 b (-1) with
...
let partition l = function
    1 -> Some [l]
    | n -> let x = weight l in if x mod n = 0 then let l2 = create_list n 0 [] in aux2 n (mergesort l) (x/n) l2 0 0 (-1) else None
;;
